...
class HeroRebirthSystem:
    ...
    def get_rebirth_stage(self, hero_name):
        return data["hero_rebirth"].get(hero_name, 0)
    
    def can_rebirth(self, hero_name):
        if "hero_management" not in data or "heroes" not in data["hero_management"]:
            return False, "武将数据不存在"
        
        hero = data["hero_management"]["heroes"].get(hero_name)
        if not hero:
            return False, "武将不存在"
    ...
    def get_rebirth_bonus(self, hero_name):
        ...
        stage_data = REBIRTH_STAGES[stage]
        bonus = {"attribute_multiplier": stage_data["attribute_multiplier"]}
        
        if "hero_management" in data and hero_name in data["hero_management"]["heroes"]:
            hero = data["hero_management"]["heroes"][hero_name]
            for skill_key in hero.get("rebirth_skills", []):
                bonus[skill_key] = REBIRTH_SKILLS[skill_key]
        ...
        while running:
            ...
            if heroes:
                ...
                if selected_hero and selected_hero in heroes:
                    ...
                    if current_stage > 0:
                        stage_data = REBIRTH_STAGES[current_stage]
                        stage_text = font_main.render(f"当前境界: {stage_data['name']} - {stage_data['description']}", True, REBIRTH_EFFECTS[current_stage]["name_color"])
                    ...
                    if selected_hero and selected_hero in heroes:
                        ...
                        if current_stage < len(REBIRTH_STAGES):
                            ...
                            if can_rebirth:
                                ...
                                if rebirth_btn_rect.collidepoint(mx, my):
