...
# 登录状态文件路径（打包后写入用户可写目录，避免写入只读临时目录）
# 先占位，稍后在导入 get_writable_base_dir 后初始化。
...
from ASSET.game_data import data, save, get_writable_base_dir
from ASSET.dictionary_system import main as dictionary_main
from ASSET.whiteboard import main as whiteboard_main

# 初始化登录状态文件路径
LOGIN_STATE_PATH = os.path.join(get_writable_base_dir(), "login_state.json")
