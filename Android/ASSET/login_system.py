...
import time
import logging
from ASSET.game_data import get_system_font_name, RESOURCES, SETTINGS, default_save, logger, get_font, draw_gradient_bg, get_writable_base_dir, sanitize_json
...
USERS_PATH = os.path.join(get_writable_base_dir(), "users.json")
...
def save_users(users):
    """保存用户数据（写入前自动消毒，避免类型异常）"""
    try:
        with open(USERS_PATH, "w", encoding="utf-8") as f:
            json.dump(sanitize_json(users), f, ensure_ascii=False, indent=2)
...
def save_game(username, save_name, user_data=None):
    ...
    if username in users:
        ...
        users[username].setdefault("saves", []).append(save_data)
